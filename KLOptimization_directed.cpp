/// TO DO: 1) allow read in from .csv 2) Allow setting args (nodes, filenames, maxedges, etc) from command line 3) only initialize directed objects if directed?
/// currently from R do something like el = as.edgelist(zach); add weights; write.table(el, file = "zach.txt", sep = "\t", col.names = FALSE, row.names = FALSE)
...
double ComputeProposal(int vertex, int from, int destination)
{
     ...
     if (!Directed) {
         ...
         for(i=0; i < ActualDiffComms; i++)
         {
               ...
               help1 = CurrentEdgeMatrix[from][NeighborIndex[i]];
               help2 = CurrentEdgeMatrix[destination][NeighborIndex[i]];
               help3 = NeighborSet[i];
        ...
        if(option == 1)
        {
            ...
            for(i=0; i < ActualDiffComms; i++)
            {
                if((NeighborIndex[i] != from) && (NeighborIndex[i] != destination))
                {
                    // do update NOTE: each community mcc' gets updated once if it had edges switch out
                    // which is correct, remembering that mcc' is symmetric and we only count c < c' here
                    BestEdgeMatrix[from][NeighborIndex[i]] -= outNeighborSet[i];
                    BestEdgeMatrix[NeighborIndex[i]][from] -= NeighborSet[i];
                    
                    BestEdgeMatrix[destination][NeighborIndex[i]] += outNeighborSet[i];
