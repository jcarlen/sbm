...
test_that("test time-dependent and not time-dependent code gives same results when T = 1", {
  ...
  expect_equal(apply(fit_dir_dc2$theta, 2, aggregate, by = list(fit_dir_dc2$FoundComms), sum)[[1]]$x, rep(1,K, 2)) #directed degree corrrection case
  expect_equal(apply(fit_dir_dc2$theta, 2, aggregate, by = list(fit_dir_dc2$FoundComms), sum)[[2]]$x, rep(1,K, 2))
